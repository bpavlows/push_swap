...
# Inicializar variáveis de cores (serão sobrescritas por style_init se necessário)
C_RESET=""
C_DIM=""
C_GREEN=""
C_RED=""
...
#   USE_VALGRIND=1 ./test_performance.sh (com detecção de memory leaks)
#
# Notas:
...
# - USE_VALGRIND=1 executa cada teste com valgrind (mais lento mas detecta leaks)

PROJECT_BIN=${PROJECT_BIN:-./push_swap}
USE_VALGRIND=${USE_VALGRIND:-0}
VALGRIND_OPTS=${VALGRIND_OPTS:---leak-check=full --show-leak-kinds=all --track-origins=yes --verbose}
...
make_bar() {
	# $1 = valor, $2 = limite, $3 = tamanho_da_barra (padrão 15)
	# Retorna uma barra visual de progresso compacta
	local val="$1" lim="$2" size="${3:-15}"
...
format_result() {
	# $1 = valor_obtido, $2 = limite_esperado, $3 = nome
	# Mostra: valor (X% do limite) | limite em uma única linha
	local val="$1" lim="$2" name="$3"
	local pct status color bar
	
	if [ "$lim" -le 0 ]; then
		printf "%s: %s (sem limite)\n" "$name" "$val"
		return
	fi
	...
	bar=$(make_bar "$val" "$lim" 12)
	
	printf "  %s%-35s%s %5s / %-5s %s\n" \
		"$color" "$name" "$C_RESET" "$val" "$lim" "$bar"
}

explain_bonus() {
	printf "\n"
	printf "%s╔════════════════════════════════════════════════════════════════╗%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                      EXPLICAÇÃO DOS RESULTADOS                  ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s╠════════════════════════════════════════════════════════════════╣%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  📊 O que cada coluna significa:                               ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • avg100: Média de operações para 100 números                 ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • avg500: Média de operações para 500 números                 ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • (X%%):   Porcentagem em relação ao limite esperado           ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • Barra:  Visualização do desempenho (█=uso, ░=margem)       ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  🎯 O que cada TIER significa:                                 ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • BONUS:  avg100 ≤ 700 E avg500 ≤ 5500 (pontuação máxima)    ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • 80%%:    uma das combinações alternativas (menos rigorosa)   ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  • FALHA:  não atende aos requisitos mínimos                  ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  📌 O que são os NÍVEIS (5..1)?                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Cada tamanho de teste (100, 500) é pontuado de 0 a 5 pontos:  ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 5 (MÁXIMO):     ops ≤ limite superior (ex: 100≤700)     ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 4:              ops ≤ 900 (para 100) ou 7000 (500)     ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 3:              ops ≤ 1100 (para 100) ou 8500 (500)    ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 2:              ops ≤ 1300 (para 100) ou 10000 (500)   ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 1:              ops ≤ 1500 (para 100) ou 11500 (500)   ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║  Nível 0 (FALHA):      ops > 1500 (para 100) ou > 11500 (500) ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                                                                ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s╚════════════════════════════════════════════════════════════════╝%s\n" "$C_BLUE" "$C_RESET"
}

show_all_performance() {
	# Mostra desempenho visual de TODOS os testes
	printf "\n"
	printf "%s╔════════════════════════════════════════════════════════════════╗%s\n" "$C_BLUE" "$C_RESET"
	printf "%s║                 📊 DESEMPENHO VISUAL DETALHADO                  ║%s\n" "$C_BLUE" "$C_RESET"
	printf "%s╚════════════════════════════════════════════════════════════════╝%s\n" "$C_BLUE" "$C_RESET"
	printf "\n"
	
	printf "%s🧮 Casos Pequenos (Obrigatório passar):%s\n\n" "$C_BLUE" "$C_RESET"
	
	if [ "$EXPECT_3" -gt 0 ]; then
		local exp3 pts3 pct3 bar3 status3 color3
		exp3="$EXPECT_3"
		pts3="-"
		if [ "$AVG_3" -le "$exp3" ]; then
			pct3=$((AVG_3 * 100 / exp3))
			status3="✅"
			color3="$C_GREEN"
		else
		...
		bar3=$(make_bar "$AVG_3" "$exp3" 10)
		printf "%s  N=3  (esperado≤%2d ops):  %s %3d / %-3d  %s%s\n" \
			"$color3" "$exp3" "$status3" "$AVG_3" "$exp3" "$bar3" "$C_RESET"
	fi
	
	if [ "$EXPECT_5" -gt 0 ]; then
		local exp5 pts5 pct5 bar5 status5 color5
		exp5="$EXPECT_5"
		pts5="-"
		if [ "$AVG_5" -le "$exp5" ]; then
			pct5=$((AVG_5 * 100 / exp5))
			status5="✅"
			color5="$C_GREEN"
		else
		...
		bar5=$(make_bar "$AVG_5" "$exp5" 10)
		printf "%s  N=5  (esperado≤%2d ops):  %s %3d / %-3d  %s%s\n" \
			"$color5" "$exp5" "$status5" "$AVG_5" "$exp5" "$bar5" "$C_RESET"
	fi
	
	printf "\n"
	printf "%s📈 Casos Grandes (Performance - Níveis 0-5):%s\n\n" "$C_BLUE" "$C_RESET"
	
	local pts100 color100 status100
	pts100=$(score_100 "$AVG_100")
	if [ "$pts100" -eq 5 ]; then
		status100="⭐⭐⭐⭐⭐ Nível 5"
		color100="$C_GREEN"
	elif [ "$pts100" -eq 4 ]; then
		status100="⭐⭐⭐⭐ Nível 4"
		color100="$C_GREEN"
	elif [ "$pts100" -eq 3 ]; then
		status100="⭐⭐⭐ Nível 3"
		color100="$C_YELLOW"
	elif [ "$pts100" -eq 2 ]; then
		status100="⭐⭐ Nível 2"
		color100="$C_YELLOW"
	elif [ "$pts100" -eq 1 ]; then
		status100="⭐ Nível 1"
		color100="$C_YELLOW"
	else
		status100="❌ Nível 0 (Falhou)"
		color100="$C_RED"
	fi
	
	local bar100 pct100
	pct100=$((AVG_100 * 100 / LIMIT_100_5))
	bar100=$(make_bar "$AVG_100" "$LIMIT_100_5" 10)
	printf "%s  N=100 (meta BONUS≤%d):    %-28s %4d / %-4d  %s%s\n" \
		"$color100" "$LIMIT_100_5" "$status100" "$AVG_100" "$LIMIT_100_5" "$bar100" "$C_RESET"
	
	printf "\n"
	
	local pts500 color500 status500
	pts500=$(score_500 "$AVG_500")
	if [ "$pts500" -eq 5 ]; then
		status500="⭐⭐⭐⭐⭐ Nível 5"
		color500="$C_GREEN"
	elif [ "$pts500" -eq 4 ]; then
		status500="⭐⭐⭐⭐ Nível 4"
		color500="$C_GREEN"
	elif [ "$pts500" -eq 3 ]; then
		status500="⭐⭐⭐ Nível 3"
		color500="$C_YELLOW"
	elif [ "$pts500" -eq 2 ]; then
		status500="⭐⭐ Nível 2"
		color500="$C_YELLOW"
	elif [ "$pts500" -eq 1 ]; then
		status500="⭐ Nível 1"
		color500="$C_YELLOW"
	else
		status500="❌ Nível 0 (Falhou)"
		color500="$C_RED"
	fi
	
	local bar500 pct500
	pct500=$((AVG_500 * 100 / LIMIT_500_5))
	bar500=$(make_bar "$AVG_500" "$LIMIT_500_5" 10)
	printf "%s  N=500 (meta BONUS≤%d):    %-28s %4d / %-4d  %s%s\n" \
		"$color500" "$LIMIT_500_5" "$status500" "$AVG_500" "$LIMIT_500_5" "$bar500" "$C_RESET"
...
count_ops() {
	...
	# Usa awk para contar linhas mesmo sem quebra final
	# Se há conteúdo vazio, retorna 0; senão conta as linhas
	awk 'END { print (NR > 0 ? NR : 0) }' 2>/dev/null
}

score_3() {
	local ops="$1"
	if ! [[ "$ops" =~ ^[0-9]+$ ]]; then echo "-"; return; fi
	if [ "$ops" -le 2 ]; then echo "5"; return; fi
	echo "0"
}
...
score_5() {
	local ops="$1"
	if ! [[ "$ops" =~ ^[0-9]+$ ]]; then echo "-"; return; fi
	if [ "$ops" -le 12 ]; then echo "5"; return; fi
	echo "0"
}
...
run_one() {
	...
	if [ "$USE_VALGRIND" -eq 1 ]; then
		valgrind_out=$(mktemp)
		out=$(valgrind $VALGRIND_OPTS --log-file="$valgrind_out" $PROJECT_BIN $args 2>&1)
		status=$?
		mem_leak=$(grep -c "definitely lost\|indirectly lost" "$valgrind_out" || echo 0)
		if grep -q "ERROR SUMMARY: 0 errors" "$valgrind_out"; then
			mem_leak=0
		else
			mem_leak=1
	...
	if [ "$SHOW_CASES" -eq 1 ]; then
		...
		if [ "$USE_VALGRIND" -eq 1 ] && [ "$mem_leak" -ne 0 ]; then
			printf "%s⚠️ MEMORY LEAK DETECTED%s\n" "$C_YELLOW" "$C_RESET" 1>&2
		fi
...
main() {
	...
	if [ "$USE_VALGRIND" -eq 1 ]; then
		if command -v valgrind >/dev/null 2>&1; then
			VALGRIND_USED=1
			printf "%s⚙️  Valgrind: ATIVADO%s (detectando memory leaks)\n" "$C_YELLOW" "$C_RESET"
		else
			printf "%s⚠️  Valgrind: NÃO ENCONTRADO%s (desativando)\n" "$C_YELLOW" "$C_RESET"
			USE_VALGRIND=0
		fi
	fi
	...
	if [ "$HAVE_CHECKER" -ne 1 ]; then
		...
		if passes_bonus_pair "$AVG_100" "$AVG_500"; then
			printf "%s✅ RESULTADO: BONUS ALCANÇADO%s\n" "$C_GREEN" "$C_RESET"
		elif passes_80_pair "$AVG_100" "$AVG_500"; then
			printf "%s✅ RESULTADO: 80%% ALCANÇADO%s\n" "$C_GREEN" "$C_RESET"
		else
			printf "%s❌ RESULTADO: ABAIXO DO ESPERADO%s\n" "$C_RED" "$C_RESET"
		fi
		
		printf "%s───────────────────────────────────────────────────────────────────%s\n" "$C_BLUE" "$C_RESET"
		printf "\n%s📈 DESEMPENHO PARA 100 NÚMEROS:%s\n" "$C_BLUE" "$C_RESET"
		format_result "$AVG_100" "$LIMIT_100_5" "  Média (tier BONUS)"
		
		printf "\n%s📊 DESEMPENHO PARA 500 NÚMEROS:%s\n" "$C_BLUE" "$C_RESET"
		format_result "$AVG_500" "$LIMIT_500_5" "  Média (tier BONUS)"
		
		printf "\n%s═══════════════════════════════════════════════════════════════════%s\n\n" "$C_BLUE" "$C_RESET"
		
		printf "%s📋 Resumo de Critérios:%s\n" "$C_BLUE" "$C_RESET"
		printf "  • BONUS necessita:  avg100 ≤ %s ops  AND  avg500 ≤ %s ops\n" "$LIMIT_100_5" "$LIMIT_500_5"
		printf "  • Seu resultado:    avg100 = %s ops  AND  avg500 = %s ops\n" "$AVG_100" "$AVG_500"
		
		if passes_bonus_pair "$AVG_100" "$AVG_500"; then
			printf "  • Status: %s✅ PASSOU em ambos os limites!%s\n" "$C_GREEN" "$C_RESET"
		elif [ "$AVG_100" -le "$LIMIT_100_5" ]; then
			printf "  • Status: ❌ avg100 OK, mas avg500 FALHOU (diferença: %s ops)\n" \
				$((AVG_500 - LIMIT_500_5))
		elif [ "$AVG_500" -le "$LIMIT_500_5" ]; then
			printf "  • Status: ❌ avg500 OK, mas avg100 FALHOU (diferença: %s ops)\n" \
				$((AVG_100 - LIMIT_100_5))
		else
			printf "  • Status: ❌ Ambos falharam\n"
		fi
	...
	if [ "$VALGRIND_USED" -eq 1 ]; then
		printf "\n"
		printf "%s════════════════════════════════════════════════════════════════%s\n" "$C_BLUE" "$C_RESET"
		if [ "$VALGRIND_LEAKS" -eq 0 ]; then
			printf "%s✅ VALGRIND: Nenhum memory leak detectado!%s\n" "$C_GREEN" "$C_RESET"
		else
			printf "%s❌ VALGRIND: %s memory leak(s) detectado(s)%s\n" "$C_RED" "$VALGRIND_LEAKS" "$C_RESET"
		fi
		printf "%s════════════════════════════════════════════════════════════════%s\n" "$C_BLUE" "$C_RESET"
	fi
}
